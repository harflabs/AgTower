...
/**
 * Whether a title is a placeholder the app generated rather than something a
 * person chose. Deliberately NARROWER than the Rust `is_auto_generated_title`
 * (engine/session_store.rs): the Rust side uses loose prefix matches and a
 * `<...>` heuristic so slug extraction can replace templated imports, but on
 * the display side those would override manual renames like "New Session plan"
 * or "fix <Foo> component" — and manual renames must always win. We match only
 * exact placeholder shapes ("New Session", "New Session 3", id-like
 * "Session abc123"), plus empty/whitespace which the UI can encounter.
 */
export function isAutoGeneratedTitle(title: string): boolean {
  const t = title.trim();
  return (
    t === "" ||
    /^New Session( \d+)?$/.test(t) ||
    /^Session [0-9a-f-]{6,}$/i.test(t) ||
    t === "Untitled CLI session" ||
    t === "Untitled Session"
  );
}

// Per-session cache for sessionDisplayTitle. Rows re-render on every PTY/token
// event while title/prompt stay the same string refs (updates spread the old
// record), so the `===` checks hit on the fast path and the regex-heavy
// derivation runs only when the inputs actually change. Keyed by session id;
// entries are tiny and the map is bounded by total session count.
const displayTitleCache = new Map<string, { title: string; prompt: string; result: string }>();

/**
 * Display title for a session: the stored title when a person (or the
 * provider's slug extractor) chose it, otherwise a title derived from the
 * session's first prompt. Existing DB rows stuck on "New Session" benefit
 * without a migration. Derivation can itself come up empty (all-noise
 * prompts return the literal "New Session"), so the result is re-checked
 * before trusting it.
 */
export function sessionDisplayTitle(session: {
  ...
  const cached = session.id ? displayTitleCache.get(session.id) : undefined;
  if (cached && cached.title === session.title && cached.prompt === session.prompt) {
    return cached.result;
  }

  let result: string;
  if (!isAutoGeneratedTitle(session.title)) {
    result = session.title;
  } else {
    const derived = session.prompt.trim()
      ? generateSessionTitle(session.prompt, session.provider)
      : "";
    result = derived && !isAutoGeneratedTitle(derived) ? derived : "Untitled Session";
  }

  if (session.id) {
    displayTitleCache.set(session.id, { title: session.title, prompt: session.prompt, result });
  }
  return result;
}
