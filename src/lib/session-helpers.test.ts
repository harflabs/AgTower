...
describe("isAutoGeneratedTitle", () => {
  it("matches the known placeholders", () => {
    expect(isAutoGeneratedTitle("New Session")).toBe(true);
    expect(isAutoGeneratedTitle("New Session 3")).toBe(true);
    expect(isAutoGeneratedTitle("Session abc123")).toBe(true);
    expect(isAutoGeneratedTitle("Untitled CLI session")).toBe(true);
    expect(isAutoGeneratedTitle("Untitled Session")).toBe(true);
    expect(isAutoGeneratedTitle("")).toBe(true);
    expect(isAutoGeneratedTitle("   ")).toBe(true);
  });

  it("treats real titles as user-chosen", () => {
    expect(isAutoGeneratedTitle("fix sidebar CSP bug")).toBe(false);
    expect(isAutoGeneratedTitle("indexed-splashing-prism")).toBe(false);
  });

  it("never classifies a rename that merely resembles a placeholder as auto", () => {
    // Rust's is_auto_generated_title uses loose startsWith prefixes and a
    // `<...>` heuristic so slug extraction can replace templated titles. On
    // the display side those would override manual renames — deliberately
    // exact placeholder shapes only here.
    expect(isAutoGeneratedTitle("fix <Foo> component")).toBe(false);
    expect(isAutoGeneratedTitle("New Session plan")).toBe(false);
    expect(isAutoGeneratedTitle("Session notes for standup")).toBe(false);
  });
});

describe("sessionDisplayTitle", () => {
  function entry(title: string, prompt: string) {
    return { title, prompt, provider: "claude-code" };
  }

  it("returns a user-chosen title untouched, even when a prompt exists", () => {
    expect(sessionDisplayTitle(entry("my rename", "do something else"))).toBe("my rename");
  });

  it("derives a title from the prompt when the stored title is a placeholder", () => {
    expect(sessionDisplayTitle(entry("New Session", "fix the sidebar contrast bug"))).toBe(
      "fix the sidebar contrast bug",
    );
  });

  it("falls back to Untitled Session when there is no prompt", () => {
    expect(sessionDisplayTitle(entry("New Session", ""))).toBe("Untitled Session");
    expect(sessionDisplayTitle(entry("New Session", "   "))).toBe("Untitled Session");
  });

  it("falls back to Untitled Session when derivation yields another placeholder", () => {
    // "/new-session" humanizes to the literal "New Session" — a derived
    // placeholder must not leak back into the UI.
    expect(generateSessionTitle("/new-session")).toBe("New Session");
    expect(sessionDisplayTitle(entry("New Session", "/new-session"))).toBe("Untitled Session");
  });
});
